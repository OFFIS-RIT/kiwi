...
export function ClarificationBlock({
    ...
    return (
        <div className="mt-3 space-y-3">
            {reason && <p className="text-sm text-muted-foreground italic">{reason}</p>}
            {questions.map((question, i) => (
                <div key={`clarification-q-${i}`} className="space-y-1">
                    <label className="text-sm font-medium" htmlFor={`clarification-textarea-${i}`}>
                        {question}
                    </label>
                    <textarea
            ...
            <div className="flex justify-end">
                {submitted ? (
                    ...
                    <Button size="sm" onClick={handleSubmit} disabled={disabled || answers.every((a) => !a.trim())}>
