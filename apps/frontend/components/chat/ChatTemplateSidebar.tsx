        ...
        <ScrollArea className="h-full">
            <div className="space-y-4 pb-2">
                {templates.map((template) => (
                    <div key={template.id} className="bg-muted/40 border-border/60 rounded-lg border p-3">
                        <div className="flex items-start justify-between gap-3">
                            <div className="space-y-1">
                                <div className="font-medium leading-tight">{template.title}</div>
                ...
                {open ? (
                    <Card className="h-full w-80 border-muted shadow-sm">
                        <CardContent className="flex h-full flex-col">
                            <SidebarPanel templates={templates} onInsert={onInsert} showHeading={false} />
                        </CardContent>
                    </Card>
                ) : null}
            </div>
