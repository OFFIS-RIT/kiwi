...
describe("connector API helpers", () => {
    ...
    test("creates a GitLab connector without rendering returned secrets", async () => {
        ...
        const post = vi.fn(async () => ({ status: "success" as const, data: { ...connector, provider: "gitlab" as const } }));
        const client = { baseURL: "/api", post } as unknown as KiwiApiClient;
    ...
    test("starts connector installation flow for a managed owner", async () => {
        const get = vi.fn(async () => ({ status: "success" as const, data: { redirectUrl: "https://github.com/apps/kiwi/install" } }));
        const client = { baseURL: "/api", get } as unknown as KiwiApiClient;
    ...
    test("fetches installation repositories and encoded branches", async () => {
        ...
        await expect(fetchConnectorBranches(client, connector.id, installation.id, repository.id)).resolves.toEqual([branch]);
    ...
    test("creates and syncs repository graphs", async () => {
        ...
        await expect(createRepositoryGraph(client, connector.id, input)).resolves.toEqual({ graph, binding, workflowRunId: "run-1" });
        await expect(syncRepositoryGraphBinding(client, binding.id)).resolves.toEqual({ binding, workflowRunId: "run-2" });
