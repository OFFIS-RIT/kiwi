...
function upsertToolPart(parts: MessagePart[], next: MessagePart) {
  ...
  const idx = parts.findIndex(
    (p) => p.type === "tool" && p.toolCallId === next.toolCallId,
  );
  if (idx === -1) {
    parts.push(next);
    return;
  ...
  .get(
    ...
    async ({ params, user, status }) => {
      if (!user) {
        return status(
          401,
          errorResponse("Unauthorized", API_ERROR_CODES.UNAUTHORIZED),
        );
      }
  ...
  .post(
    ...
    async ({ params, body, user, status }) => {
      ...
      const replyResult = await Result.tryPromise(async () => {
        ...
        await assertCanViewGraph(user, params.id);
        const { assistantId, client, tools, prompt } = await startReply(
          user.id,
          params.id,
          request,
        );
      ...
      const streamResult = await Result.tryPromise(async () => {
        ...
        const stream = createUIMessageStream<ChatUIMessage>({
          execute: async ({ writer }) => {
            ...
            const createUITextId = (modelPartId: string) =>
              `${modelPartId}::${uiTextBlockCounter++}`;
              ...
              for await (const part of result.fullStream) {
                ...
                switch (part.type) {
                  ...
                  case "tool-error": {
                    ...
                    upsertToolPart(
                      assistantParts,
                      toolPart(part, "failed", { value: errorText }),
                    );
                    await updateAssistantMessage(
                      assistantId,
                      assistantParts,
