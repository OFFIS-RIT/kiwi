...
export const findActiveDeleteGraphFilesProgress: (
    ...
    tryDb((db) =>
        Effect.gen(function* () {
            ...
            const runs = (yield* db.execute(sql<DeleteWorkflowRun>`
                SELECT "id" AS "id",
            ...
            const [fileProgressRows, descriptionProgressRows] = yield* Effect.all(
                [
                    Effect.map(
                        db.execute(sql<DeleteFileProgressRow>`
                            ...
                            GROUP BY parent."id"
                        `),
                        (rows) => rows as DeleteFileProgressRow[]
                    ),
                    Effect.map(
                        db.execute(sql<DeleteDescriptionProgressRow>`
                            ...
                            GROUP BY parent."id"
                        `),
                        (rows) => rows as DeleteDescriptionProgressRow[]
                    ),
                ],
