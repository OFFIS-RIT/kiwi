...
describe("project file proxy URLs", () => {
    test("builds page-aware proxy paths", () => {
        ...
        expect(getProjectFileProxyPath("graph-1", "file-1", { page: 3, token: "abc.123" })).toBe(
            "/graphs/graph-1/files/file-1?token=abc.123#page=3"
        );
    ...
    test("prefixes proxy paths with an API origin", () => {
        expect(getProjectFileProxyUrl("https://api.example.com/", "graph-1", "file-1", { page: 2 })).toBe(
            "https://api.example.com/graphs/graph-1/files/file-1#page=2"
        );
    });
