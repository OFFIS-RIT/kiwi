        ...
        Effect.catchDefect(
            Effect.gen(function* () {
                ...
                if (unit.file_type !== "pdf") {
                    return yield* Effect.fail(
                        makeApiError(415, API_ERROR_CODES.UNSUPPORTED_FILE_TYPE, "Unsupported file type")
                    );
                }
                ...
                if (startPage === null || endPage === null || page < startPage || page > endPage) {
                    return yield* Effect.fail(
                        makeApiError(422, API_ERROR_CODES.INVALID_PAGE_RANGE, "Invalid page range")
                    );
                }
                ...
                if (previewResult.status === "page_missing") {
                    return yield* Effect.fail(
                        makeApiError(404, API_ERROR_CODES.INVALID_PAGE_RANGE, "PDF preview page not found")
                    );
                }
