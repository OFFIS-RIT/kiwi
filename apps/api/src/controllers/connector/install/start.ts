...
export function startConnectorInstall(input: {
    user: AuthUser;
    connectorId: string;
    query: ConnectorConnectQuery;
}): Effect.Effect<{ redirectUrl: string }, ReturnType<typeof toApiError>, Database> {
    return Effect.mapError(
        Effect.gen(function* () {
            ...
            return yield* Effect.fail(
                new Error("GitLab connector installations are disabled until OAuth flow support lands.")
            );
        }),
