...
export function getConnectorGraphBinding(input: {
    user: AuthUser;
    bindingId: string;
}): Effect.Effect<ReturnType<typeof toBindingResponse>, ReturnType<typeof toApiError>, Database> {
    return Effect.mapError(
        Effect.map(assertCanViewBinding(input.user, input.bindingId), ({ binding }) => toBindingResponse(binding)),
