...
ALTER TABLE "chats" ADD CONSTRAINT "chats_scope_target_check" CHECK ((("scope" = 'graph' AND "project_id" IS NOT NULL AND "team_id" IS NULL) OR ("scope" = 'team' AND "project_id" IS NULL AND "team_id" IS NOT NULL))) NOT VALID;--> statement-breakpoint
ALTER TABLE "chats" VALIDATE CONSTRAINT "chats_scope_target_check";--> statement-breakpoint
