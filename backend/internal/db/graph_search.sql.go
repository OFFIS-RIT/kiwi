...
const findRelevantEntities = `-- name: FindRelevantEntities :many
WITH top_sources AS (
    SELECT
        s.id,
        s.entity_id,
...
type FindRelevantEntitiesParams struct {
	Embedding pgvector.Vector `json:"embedding"`
	Limit     int32           `json:"limit"`
	ProjectID int64           `json:"project_id"`
}

type FindRelevantEntitiesRow struct {
	EntityID            int64       `json:"entity_id"`
	EntityName          string      `json:"entity_name"`
	RelevantSourceCount int64       `json:"relevant_source_count"`
	AvgDistance         float64     `json:"avg_distance"`
	BestDistance        interface{} `json:"best_distance"`
}

func (q *Queries) FindRelevantEntities(ctx context.Context, arg FindRelevantEntitiesParams) ([]FindRelevantEntitiesRow, error) {
	rows, err := q.db.Query(ctx, findRelevantEntities, arg.Embedding, arg.Limit, arg.ProjectID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []FindRelevantEntitiesRow{}
	for rows.Next() {
		var i FindRelevantEntitiesRow
		if err := rows.Scan(
			&i.EntityID,
			&i.EntityName,
			&i.RelevantSourceCount,
			&i.AvgDistance,
			&i.BestDistance,
		); err != nil {
			return nil, err
...
const getEntityNeighbours = `-- name: GetEntityNeighbours :many
SELECT r.id, e.id
...
type GetEntityNeighboursRow struct {
	ID   int64 `json:"id"`
	ID_2 int64 `json:"id_2"`
}

func (q *Queries) GetEntityNeighbours(ctx context.Context, sourceID int64) ([]GetEntityNeighboursRow, error) {
	rows, err := q.db.Query(ctx, getEntityNeighbours, sourceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetEntityNeighboursRow{}
	for rows.Next() {
		var i GetEntityNeighboursRow
