...
// GetGlobalQueryContext retrieves context for global queries by finding entities
// semantically similar to the query embedding. It returns formatted context text
// containing relevant entity sources, or an empty string if no relevant context
// is found.
func (s *GraphDBStorage) GetGlobalQueryContext(ctx context.Context, query string, embedding []float32, graphId string) (string, error) {
	...
	relevantEntityIds, err := s.getSimilarEntityIdsByEmebedding(ctx, qtx, projectId, embedding, 10)
	if err != nil {
		return "", err
	}

	if len(relevantEntityIds) == 0 {
		return "", nil
	}

	embed := pgvector.NewVector(embedding)
	entitySources, err := qtx.FindRelevantEntitySources(ctx, db.FindRelevantEntitySourcesParams{
...
func (s *GraphDBStorage) GetLocalQueryContext(
	...
	relevantEntities, err := qtx.GetProjectEntitiesByNames(ctx, db.GetProjectEntitiesByNamesParams{
		ProjectID: projectId,
	...
	additionalEntitySources, err := qtx.FindRelevantEntitySources(ctx, db.FindRelevantEntitySourcesParams{
	...
	additionalSources := make([]db.FindSimilarEntitySourcesRow, 0)
	dbAdditionalSources, err := qtx.FindSimilarEntitySources(ctx, db.FindSimilarEntitySourcesParams{
		Embedding: embed,
		ProjectID: projectId,
		Limit:     10,
		Column4:   0.4,
	...
	relationshipSources, err := qtx.FindRelevantRelationSources(ctx, db.FindRelevantRelationSourcesParams{
	...
	pathSources, err := qtx.FindRelevantEntitySources(ctx, db.FindRelevantEntitySourcesParams{
