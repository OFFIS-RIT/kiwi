...
func toolSearchEntities(conn *pgxpool.Pool, aiClient ai.GraphAIClient, projectId int64) ai.Tool {
	return ai.Tool{
		...
		Handler: func(ctx context.Context, args string) (string, error) {
			...
			q := db.New(conn)
			entities, err := q.SearchEntitiesByEmbedding(ctx, db.SearchEntitiesByEmbeddingParams{
				ProjectID: projectId,
...
func toolGetEntityNeighbours(conn *pgxpool.Pool, aiClient ai.GraphAIClient) ai.Tool {
	return ai.Tool{
		...
		Handler: func(ctx context.Context, args string) (string, error) {
			...
			q := db.New(conn)
			neighbours, err := q.GetEntityNeighboursRanked(ctx, db.GetEntityNeighboursRankedParams{
				SourceID:  entityId,
				Embedding: pgvector.NewVector(embedding),
				Limit:     limit,
			})
...
func toolGetEntitySources(conn *pgxpool.Pool, aiClient ai.GraphAIClient) ai.Tool {
	return ai.Tool{
		...
		Handler: func(ctx context.Context, args string) (string, error) {
			...
			q := db.New(conn)
			sources, err := q.FindRelevantSourcesForEntities(ctx, db.FindRelevantSourcesForEntitiesParams{
				Column1:   entityIds,
				Embedding: pgvector.NewVector(embedding),
				Limit:     limit,
			})
...
func toolGetRelationshipSources(conn *pgxpool.Pool, aiClient ai.GraphAIClient) ai.Tool {
	return ai.Tool{
		...
		Handler: func(ctx context.Context, args string) (string, error) {
			...
			q := db.New(conn)
			sources, err := q.FindRelevantSourcesForRelations(ctx, db.FindRelevantSourcesForRelationsParams{
...
func toolSearchEntitiesByType(conn *pgxpool.Pool, aiClient ai.GraphAIClient, projectId int64) ai.Tool {
	return ai.Tool{
		...
		Handler: func(ctx context.Context, args string) (string, error) {
			...
			q := db.New(conn)
			entities, err := q.SearchEntitiesByType(ctx, db.SearchEntitiesByTypeParams{
				ProjectID: projectId,
				Type:      entityType,
				Embedding: pgvector.NewVector(embedding),
				Limit:     limit,
